...
class PreProcessingDBHandler:
	...
	def __init__(self):
		...
		self.clean_htmls_table = "select id,clean_html,pp_url from privacy_policy where url_return_code=200 and process_status='HTML_CLEANED'"
	...
	def insert_db_http_ok (self, url_record, pp_html):
		...
		self.db_util.exec_command("INSERT INTO privacy_policy \
	...
	def update_html_cleaned(self, result, pp_html_record):
		...
		self.db_util.exec_command("UPDATE privacy_policy SET \
							 process_status = %s,process_status_details = %s,clean_html = %s \
							 where id=%s", db_rows)


	def insert_pp_paragraphs(self, db_rows):
		self.db_util.exec_command("INSERT INTO privacy_policy_paragraphs \
                                       (paragraph,pp_url,index,privacy_policy_id) \
                                       VALUES \
                                       (%s,%s,%s,%s)", db_rows)

	def pp_splitted_ok(self, html_record):
